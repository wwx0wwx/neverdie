...
export function getAgents(): AgentConfigRecord[] {
  const stmt = db.prepare('SELECT * FROM agents WHERE deletedAt IS NULL ORDER BY baseUrl');
  return (stmt.all() as any[]).map(rowToRecord);
}

export function getAllRecords(): AgentConfigRecord[] {
  const stmt = db.prepare('SELECT * FROM agents ORDER BY baseUrl');
  return (stmt.all() as any[]).map(rowToRecord);
}

export function upsertAgent(record: AgentConfigRecord): void {
...
export function softDeleteAgent(baseUrl: string, updatedAt: string): void {
...
export function mergeAgents(records: AgentConfigRecord[]): void {
  const stmt = db.prepare(`
    INSERT INTO agents (baseUrl, displayName, enabled, updatedAt, deletedAt)
    VALUES (?, ?, ?, ?, ?)
    ON CONFLICT(baseUrl) DO UPDATE SET
...
function rowToRecord(row: any): AgentConfigRecord {
  return {
    baseUrl: row.baseUrl,
...
export function getAdmin(username: string): { id: number; username: string; passwordHash: string } | undefined {
  return db.prepare('SELECT id, username, passwordHash FROM admins WHERE username = ?').get(username) as any;
}

// --- Panel peer functions ---

export function getPanels(): PanelRecord[] {
  return (db.prepare('SELECT * FROM panels WHERE deletedAt IS NULL ORDER BY joinedAt').all() as any[]).map(
    (r) => ({ id: r.id, displayName: r.displayName ?? undefined, domain: r.domain, secret: r.secret, joinedAt: r.joinedAt, deletedAt: r.deletedAt ?? undefined }),
  );
}

export function upsertPanel(record: PanelRecord): void {
  db.prepare(`
...
export function softDeletePanel(id: string, deletedAt: string): void {
  db.prepare('UPDATE panels SET deletedAt = ? WHERE id = ?').run(deletedAt, id);
}

// --- Agent key functions ---

export function getAgentKeys(): AgentKeyRecord[] {
  return (db.prepare('SELECT * FROM agent_keys WHERE deletedAt IS NULL ORDER BY createdAt').all() as any[]).map(
    (r) => ({ agentId: r.agentId, displayName: r.displayName ?? undefined, secret: r.secret, baseUrl: r.baseUrl ?? undefined, createdAt: r.createdAt, deletedAt: r.deletedAt ?? undefined }),
  );
}

export function upsertAgentKey(record: AgentKeyRecord): void {
  db.prepare(`
...
export function softDeleteAgentKey(agentId: string, deletedAt: string): void {
